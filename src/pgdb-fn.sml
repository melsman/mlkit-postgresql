(* Generic functor to be applied to different representations of sql
   data, for instance strings or quotations.

*)

functor PgDbFn (X: sig type sql
                       val sql2s : sql -> string
                       val s2sql : string -> sql
                   end) : DB =
struct

  fun seqNextvalExp seqn = "nextval('" ^ seqn ^ "')"
  fun seqCurrvalExp seqn = "currval('" ^ seqn ^ "')"

  structure PgDbHandle : DB_HANDLE =
  struct
    ...
    fun dml c q = Pg.exec(c,sql2s q)

    fun errmsg c = Pg.errmsg c

    fun fold (c:db) (f:(string->string)*'a->'a) (a:'a) (q:sql) : 'a =
        let val res = Pg.query (c, sql2s q)
    ...
    fun foldRaw (c:db) (f:string option list*'a->'a) (a:'a) (q:sql) : 'a =
        let val res = Pg.query (c, sql2s q)
            val n = Pg.ntuples res
            val m = Pg.nfields res
            fun loop (i, a) =
    ...
    (* Transactions *)
    fun begin (c:db) : unit = Pg.exec(c,"BEGIN")
    fun commit (c:db) : unit = Pg.exec(c,"COMMIT")
    fun rollback (c:db) : unit = Pg.exec(c,"ROLLBACK")
    fun transaction (c:db) (f : db -> 'a) : 'a =
        (begin c; f c before commit c)
        handle ? => (rollback c; raise ?)

    (* Sequences *)
    fun seqNextval c (seqName:string) : int =
	let val s = oneField c (s2sql ("select " ^ seqNextvalExp seqName))
	...
	let val s = oneField c (s2sql ("select " ^ seqCurrvalExp seqName))
	in case Int.fromString s of
	       SOME i => i
  ...
  struct
    ...
    (* Connectivity *)
    local val conn : Pg.db option ref = ref NONE
    ...
    fun transactionConn (f : Handle.db -> 'a) : 'a =
